...
(function () {
	...
	function initCustomSelect(select) {
		...
		if (select.arrowIcon.length > 0)
			select.arrowIcon[0].style.display = 'none';
	...
	function selectOption(select, option) {
		if (
			...
			select.trigger.getElementsByClassName(
				'js-select__label'
	...
	function initListSelect(select) {
		...
		if (select.optGroups.length > 0) {
			for (var i = 0; i < select.optGroups.length; i++) {
				var optGroupList = select.optGroups[i].getElementsByTagName(
						'option'
					),
					optGroupLabel =
	...
	function getSelectedOption(select) {
		...
		if (option) return option;
			...
			return select.dropdown.getElementsByClassName('js-select__item')[0];
	}
	...
	function checkCustomSelectClick(select, target) {
		...
		if (!select.element.contains(target))
			toggleCustomSelect(select, 'false');
	}
